...
int op,n,cont=0,Dato;

main(){
	do{
		panta2();
		gt(33,9);co "MENU - ARBOLES";
		...
		gt(35,18);co "8. SALIR";
		gt(35,20); co "OPCI"<<char(224)<<"N: ";ci op;
		...
		switch(op){
			...
			case 7:panta2();
				...
				postord(arbol);
				gt(20,20);system("pause");break;
			case 8: gt(34,20)co "HASTA LUEGO ;)";gt(1,24)exit(0);
...
void inord(Nodo *arbol){//recorrido preorden(sub arbolizq - raiz - sub arbolder)
	if(arbol==NULL){
	return;
	}else{
...
void postord(Nodo *arbol){//recorrido preorden(sub arbolizq - sub arbolder - raiz)
	if(arbol==NULL){ 
	return;
	}else{
